---
title: all things ssh - chapter 2
date: 2026-07-09
...
<figure class="img-s"><img alt="" class="kg-image" loading="lazy" src="https://media.giphy.com/media/v1.Y2lkPTc5MGI3NjExaDZtcGxhbzQ4NXNvMnVlcXp2dDN4YWx3YW43MG14b3k1aHVvczc1cyZlcD12MV9naWZzX3NlYXJjaCZjdD1n/KbdF8DCgaoIVC8BHTK/giphy.gif"/></figure>

So today we're not talking about ssh as "the thing you use to log into your homelab." We're talking about ssh as a pivoting engine! The tool that turns one compromised box into a doorway for every network sitting behind it. If you've ever fooled around with <u>HTB Lab Environments</u>  with more than one subnet, you already know where this is going.

https://help.hackthebox.com/en/articles/5185470-how-to-play-pro-labs#h_29a2f67cf2

> [!note] Quick Disclaimer
> Port forwarding and tunneling are neutral tools. Whether they're <u>hacking</u> or <u>system administration</u> depends entirely on whether you were invited. Snooping around corporate networks can quickly turn a fun Tuesday into a conversation with your lawyer.
...
Every internal network I've ever poked at has had ssh sitting somewhere, usually on a Linux box that's dual-homed between two network segments, quietly bridging a network the firewall would otherwise never let you touch. A few reasons ssh keeps showing up on both sides of the fence:
...
- It does four completely different jobs. Shell access, file transfer, port forwarding, and SOCKS proxying. All with the same binary and the same credentials.

That last point is the whole reason this post exists. Once you have a foothold with ssh access, you effectively have a Swiss Army knife for reaching everything that box can reach, without <u>dropping a single extra tool on disk</u>.
...
`confluence01` is your beachhead. You popped a shell on it through some exposed web app, and it happens to be dual-homed, sitting on both your network and the `10.4.50.0/24` internal range. From there, `pgdatabase01` is reachable, and it turns out *that* box is also dual-homed into a third, even more internal network where `hr_shares` lives with port 445 open. The Classic onion.

None of these techniques care whether the "compromise" step was a <u>web shell</u>, a <u>reverse shell</u>, or you just SSH'd in with <u>harvested creds</u>. The moment you have command execution and an ssh client on the pivot box, the rest of this applies.
...
Worth knowing that `qemu-x86_64-static` doesn't understand your `$PATH` so you might always want to give it absolute paths, or it'll stare back at you blankly.
...
Sometimes you don't want a SOCKS proxy, you want something closer to an actual VPN with full subnet routing without wrapping every single command in `proxychains`. That's what `sshuttle` is for.
...
If for some reason the box doesn't have OpenSSH but does have PuTTY's command-line sibling, `plink` covers most of the same ground. <u>Local</u>, <u>dynamic</u>, and <u>remote</u> forwarding all work. The one thing it **can't** do is remote *dynamic* forwarding, so if that's the technique your pivot chain needs, plink alone won't get you there and you'll need to find another way to drop a real ssh client.
...
The hardening post, locking down `sshd_config` so none of this works against *you*, is still coming. I'm not going to promise a date this time. Hehehe :)

<figure class="img-xs"><img alt="" class="kg-image" loading="lazy" src="https://media2.giphy.com/media/v1.Y2lkPTc5MGI3NjExand2ejZxcjg3N3RibG1ha2dtdWhyYnV6Z3VtdWJmYXFobGpwN3FwdyZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/JNIfA0WTiI3moj7zT1/giphy.gif"/></figure>
